...
and torus $Q=2$. Generated by \texttt{experiments/\allowbreak figures/\allowbreak make\_\allowbreak genesis\_\allowbreak figures.py} from the
logs \texttt{bp\_\allowbreak quantization\_\allowbreak 20260709} and \texttt{referee\_\allowbreak response\_\allowbreak tests\_\allowbreak 20260709}.}
...
$\pi_2(G/H)\cong\pi_1(H)$ --- the same identity that classifies 't~Hooft--Polyakov monopoles~\cite{thooft1974,polyakov1974}.
...
(Kibble--Zurek~\cite{kibble1976,zurek1985} relic fraction $\approx1.85\%$, Appendix~Y).

\paragraph{A spectral jewel of the condensation stage.} \textbf{[LIT]}
$S^6\cong G_2/SU(3)$ carries $O(7)$ symmetry, and the fermionic (Gross--Neveu/NJL-type~\cite{grossneveu1974,njl})
...
not chosen. \textbf{[THM]} Derrick's theorem~\cite{derrick1964}: in $3d$ the pure Dirichlet energy scales as
...
proper-time integral exactly), the Skyrme combination~\cite{skyrme1961} appears with positive coefficient, and
...
integral~\cite{whitehead1947} computed on explicit $(p,q)$ rational-map configurations ($96^3$ lattice,
\texttt{experiments/\allowbreak layer0/\allowbreak hopf\_\allowbreak lift\_\allowbreak charge\_\allowbreak algebra.py}) confirms
\begin{equation}
...
higher charge \cite{battye1998,Sutcliffe2007}. Converting Sutcliffe's normalization
...
Skyrme-physics pattern (baryon mass $\sim N_c\times$ constituent mass)~\cite{witten1979}.
...
$M(1,1)=184\text{--}201$\,TeV. \emph{Production (2026-08-14 computation, log
\texttt{gap\_n4d\_relic\_20260814}):} Kibble--Zurek~\cite{kibble1976,zurek1985} defect formation at the Great
Condensation overproduces the tower by $8$--$11$ orders, and geometric
soliton--antisoliton annihilation~\cite{preskill1979} (solitons are extended, size $\sim 1/M^*$; thermal
re-creation exponentially suppressed) then erases the \emph{symmetric} component to
$\Omega h^2 \sim 10^{-18}$--$10^{-12}$, robustly across all uncertainty bands.
\textbf{The tower is therefore viable as the dark matter only through a primordial
topological asymmetry} with the exact target yield
$Y_\Delta = (\Omega_{\rm DM}/\Omega_B)(m_p/M)\,Y_B = 2.4\times10^{-15}$ --- a cogenesis
mechanism suppressed relative to baryogenesis by $m_p/M(1,1)$ (register item
GAP-N4d-asym). This sharpens the phenomenology: asymmetric dark matter~\cite{zurek2014} predicts
\emph{no annihilation signals today}, aligning with the indirect-detection nulls this
report had (Section~\ref{sec:dark_matter_v12}) attached to the superseded 5.7\,GeV
candidate. (An earlier draft's ``unitarity-bound~\cite{griest1990} thermal relic'' phrasing is retracted:
thermal equilibration from below is unavailable to coherent solitons.)
