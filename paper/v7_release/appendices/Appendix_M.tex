...
\begin{itemize}
    \item \textbf{Mass:} $m_{DM} = M^* \times (Area-Overlap Form Factor) = 5.71$ GeV.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/fig_relic_abundance.png}
    \caption{\textbf{Dark Matter Relic Abundance Constraints.} 
    Parameter space for TRXT Phonon-mediated SIDM. The Planck 2018 constraint ($\Omega h^2 \approx 0.12$) is satisfied along the white dashed band. 
    Red stars indicate parameter points found by the analytical scanner that match Planck data while satisfying SIDM self-interaction bounds ($\sigma/m \sim 0.1-1$ cm$^2$/g for galaxies). 
    Consistency is achieved for natural couplings $\alpha_{DM} \sim 10^{-4} - 10^{-2}$.}
    \label{fig:relic_abundance}
\end{figure}
